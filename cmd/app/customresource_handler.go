...
func (r *crdHandler) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if !ok {
		responsewriters.ErrorNegotiated(
			...
			apiextensionsapiserver.Codecs, schema.GroupVersion{Group: requestInfo.APIGroup, Version: requestInfo.APIVersion}, w, req,
		)
		return
	}
	...
	crd, err := r.getTenantCRD(req.Context(), crdName)
	if apierrors.IsNotFound(err) {
		if !r.hasSynced() {
			responsewriters.ErrorNegotiated(serverStartingError(), apiextensionsapiserver.Codecs, schema.GroupVersion{Group: requestInfo.APIGroup, Version: requestInfo.APIVersion}, w, req)
			return
		}
	...
	crdInfo, err := r.getOrCreateServingInfoFor(req.Context(), crd.UID, crd.Name)
	if apierrors.IsNotFound(err) {
		r.delegate.ServeHTTP(w, req)
		return
	}
...
func (r *crdHandler) serveResource(w http.ResponseWriter, req *http.Request, requestInfo *apirequest.RequestInfo, crdInfo *crdInfo, terminating bool, supportedTypes []string) http.HandlerFunc {
	...
	proxyStorage := proxy.(rest.StandardStorage)

	switch requestInfo.Verb {
	case "get":
		return handlers.GetResource(proxyStorage, nil, requestScope)
	case "list":
		forceWatch := false
		return handlers.ListResource(proxyStorage, proxyStorage, requestScope, forceWatch, r.minRequestTimeout)
	case "watch":
		forceWatch := true
		return handlers.ListResource(proxyStorage, proxyStorage, requestScope, forceWatch, r.minRequestTimeout)
	case "create":
		if terminating {
			...
			responsewriters.ErrorNegotiated(err, apiextensionsapiserver.Codecs, schema.GroupVersion{Group: requestInfo.APIGroup, Version: requestInfo.APIVersion}, w, req)
			return nil
		}
		return handlers.CreateResource(proxyStorage, requestScope, nil)
	case "update":
		return handlers.UpdateResource(proxyStorage, requestScope, nil)
	case "patch":
		return handlers.PatchResource(proxyStorage, requestScope, nil, supportedTypes)
	case "delete":
		allowsOptions := true
		return handlers.DeleteResource(proxyStorage, allowsOptions, requestScope, nil)
	case "deletecollection":
		checkBody := true
		return handlers.DeleteCollection(proxyStorage, checkBody, requestScope, nil)
	default:
		responsewriters.ErrorNegotiated(
...
func (r *crdHandler) serveStatus(w http.ResponseWriter, req *http.Request, requestInfo *apirequest.RequestInfo, crdInfo *crdInfo, terminating bool, supportedTypes []string) http.HandlerFunc {
	...
	switch requestInfo.Verb {
	case "get":
		return handlers.GetResource(proxyStorage.(rest.Getter), nil, requestScope)
	case "update":
		return handlers.UpdateResource(proxyStorage.(rest.Updater), requestScope, nil)
	case "patch":
		return handlers.PatchResource(proxyStorage.(rest.Patcher), requestScope, nil, supportedTypes)
	default:
		responsewriters.ErrorNegotiated(
			apierrors.NewMethodNotSupported(schema.GroupResource{Group: requestInfo.APIGroup, Resource: requestInfo.Resource}, requestInfo.Verb),
			apiextensionsapiserver.Codecs, schema.GroupVersion{Group: requestInfo.APIGroup, Version: requestInfo.APIVersion}, w, req,
...
func (r *crdHandler) GetCustomResourceListerCollectionDeleter(crd *apiextensionsv1.CustomResourceDefinition) (finalizer.ListerCollectionDeleter, error) {
	info, err := r.getOrCreateServingInfoFor(context.TODO(), crd.UID, crd.Name)
	if err != nil {
		return nil, err
	}
...
func (r *crdHandler) getOrCreateServingInfoFor(ctx context.Context, uid types.UID, name string) (*crdInfo, error) {
	...
	crd, err := r.getTenantCRD(ctx, name)
	if err != nil {
		return nil, err
	}
	storageMap = r.customStorage.Load().(crdStorageMap)
...
func (r *crdHandler) getTenantCRD(ctx context.Context, name string) (*apiextensionsv1.CustomResourceDefinition, error) {
	tenantID, exists := util.TenantFrom(ctx)
	if !exists {
	...
	if !strings.HasPrefix(crd.Spec.Group, tenantID) {
		return nil, fmt.Errorf("invalid spec.group: %s for crd: %s, must have tenantID prefix: %s", crd.Spec.Group, crd.Name, tenantID)
	}
	// notice: Do Not pollute the original crd in crd informer
	crd = crd.DeepCopy()
